...
static void lcd_loop(Lcd_HandleTypeDef* lcd)
{
	if(HAL_GetTick()-lcd_tick > 1000)
	{
		Lcd_home(lcd);
		sprintf(buf,"Distance = %u  ",hcsr04.distance);
		Lcd_string(lcd, buf);
		Lcd_cursor(lcd, 1, 0);
		sprintf(adc_buf,"TempSens = %u  ",MyAdc.temperature);
		Lcd_string(lcd, adc_buf);
	}
}
*/
...
int main(void)
{
  /* USER CODE BEGIN 1 */
	...
	//char spi_buf[20];
	//uint16_t len = sprintf(spi_buf,"Hello i m master!");
	//UNUSED(len),
